...
<template>
 <div v-if="authStore.isAuth">
   <transition-group name="list" tag="div" class="-space-x-3 hover-hover:-space-x-7 hover:-space-x-3 rtl:space-x-reverse my-9">
     <div class="flex items-center justify-center gap-2" v-if="friendStore.isLoading">
       <div class="skeleton h-32 w-full"></div>
     </div>
     <div class="flex items-center justify-center gap-2" v-else-if="!friendStore.items.length">
     ...
     <div v-else class="tooltip ring rounded-full ring-white cursor-pointer transition-all" :data-tip="friend.email" v-for="friend in friendStore.items" :key="friend.username">
